...
export default function TimerDisplay({ timer, editMode, className, editModeFirstRender }: TimerDisplayProps) {
    ...
    return (
        <div className={`flex align-baseline transition-all border-b border-nord-4 ${editModeFirstRender ? "opacity-50" : ""} ${editMode && !editModeFirstRender ? "border-opacity-70" : "border-opacity-0"}`}>
            {displayHours &&
                <div className="flex">
