...
export default function AppSettings({
  ...
  return (
    <div className="relative flex flex-col gap-4">
      <div className="bg-nord-1 sticky top-0 z-50 flex w-full gap-4 border-t border-opacity-60 p-4 shadow-lg">
        <Button onClick={createNewApp} colour="green">
          New app
        </Button>
                    ...
                    <Draggable draggableId={app.id} index={index} key={app.id}>
                      {(provided) => (
                        <div
                          ...
                          ref={provided.innerRef}
                          className="mb-4 max-w-[96rem] self-start"
                        >
                          <AppForm
                            app={app}
                            editEnabled={editEnabledAppId === app.id}
                            enableEditing={setEditEnabledAppId}
                            config={config}
