...
export async function cacheWeatherData(newWeatherdata: WeatherData, location: string) {
    const cacheData = await getCacheData();
    // Initialize cachedWeatherData
    if (!cacheData.cachedWeatherData)
        cacheData.cachedWeatherData = [];

    // Remove existing cache for the location
    cacheData.cachedWeatherData = cacheData.cachedWeatherData.filter(x => x.location.toLowerCase().trim() != location.toLowerCase().trim());
